...
def decode(loc, priors, variances):
    #loc = loc.to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    #priors = priors.to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    priors = priors.cpu()
    boxes = torch.cat((
...
def ssd_correct_boxes(top, left, bottom, right, input_shape, image_shape):
    ...
    new_shape = np.array(image_shape) * np.min(input_shape / image_shape)  # 300/1330 * 1330 = 300
