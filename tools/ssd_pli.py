...
class SSD(object):
    ...
    def detect_image(self, image):
        ...
        boxes = ssd_correct_boxes(top_ymin, top_xmin, top_ymax, top_xmax,
                                  np.array([self.opt.input_shape, self.opt.input_shape]), image_shape[0:2])
        ...
        for i, c in enumerate(top_label):
            ...
            bottom = min(np.shape(image)[0], np.floor(bottom + 0.5).astype('int32'))  # y2
            right = min(np.shape(image)[1], np.floor(right + 0.5).astype('int32'))  # x2
            axis = [top, left, bottom, right]

            # img_reco_crop = Second_Detec(image1,top,left,bottom,right)  # Image格式
            # img_reco_crop = ssd1.detect_image(img_reco_crop)
