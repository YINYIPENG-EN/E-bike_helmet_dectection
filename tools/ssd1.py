...
sys.path.append('../')
from EC.nets import ssd
from EC.utils.box_utils import letterbox_image, ssd_correct_boxes
from EC.nets.ssd_layers import Detect
from EC.utils.config import Config
...
class SSD1(object):
    _defaults = {
        "classes_path": 'EC/model_data/new_classes.txt',
    ...
    def generate(self):
        ...
        # trt加载
        if self.opt.trt:
            self.net = DetectMultiBackend(self.opt.helmet_weights, device, False)
        else:
            model = torch.load(self.opt.helmet_weights, map_location=device)
        ...
        print('{} model, anchors, and classes loaded.'.format(self.opt.helmet_weights))
        # 画框设置不同的颜色
    ...
    def detect_image(self, image):
        ...
        with torch.no_grad():
            ...
            image = letterbox(image, self.opt.input_shape2)  # 这里的letterbox采用的yolov5的
        ...
        boxes = ssd_correct_boxes(top_ymin, top_xmin, top_ymax, top_xmax,
                                  np.array([self.opt.input_shape2, self.opt.input_shape2]), image_shape)

        thickness = max((np.shape(image)[0] + np.shape(image)[1]) // self.opt.input_shape2, 1)
